...
var editId=null,selRegions=new Set(['fr']);
...
function renderRegions(){
  ...
  REGIONS.forEach(function(r){
    ...
    el.innerHTML='<div class="region-top"><span class="region-name">'+r.name+'</span></div><div class="region-flags">'+r.flags.map(function(f){return'<span class="region-flag">'+f+'</span>'}).join('')+'</div>';
    el.addEventListener('click',function(){
      ...
      else{if(selRegions.size>=3){toast('Максимум 3 страны');return}selRegions.add(r.id);el.classList.add('active')}
    });g.appendChild(el);
  });
}
...
function renderSearches(){
  ...
  searches.forEach(function(s){
    ...
    card.innerHTML='<div class="s-top"><div><div class="s-title">'+esc(s.query||'—')+'</div><div class="s-meta">'+esc(doms)+'</div></div><span class="s-badge">Активен</span></div><div class="s-actions"><button class="s-btn ed">Изменить</button><button class="s-btn danger dl">Удалить</button></div>';
...
function cancelEdit(){
  ...
  $('#searchForm').reset();selRegions.clear();selRegions.add('fr');renderRegions();
}
...
$('#searchForm').addEventListener('submit',function(e){
  ...
  var data={query:q,domain:getDomains(),category:$('#category').value,size:$('#size').value.trim(),
    ...
    minus_words:$('#minus_words').value.trim(),conditions:[]};
  if(editId){data.action='edit';data.search_id=editId;toast('Сохраняю...')}else toast('Создаю...');
  send(data);
});

renderRegions();renderProfile();renderSearches();
})();